...
- 新增 CodeAct 执行范式：智能体新增 `use_code_act` 静态配置开关（`ChatBotContext`，前端配置页自动渲染 boolean 开关，默认关闭，存量智能体零变化），开启后挂载 `CodeActMiddleware` 并注入 `execute_python` 工具，让模型写一段 Python 代码在沙盒执行完成多步组合任务（批量检索/循环处理/数据加工），减少 ReAct 逐步工具调用的轮次。**工具桥回调**——沙盒内代码经注入的 `starring_tools.py`（仅标准库 urllib）调用 `starring_tools.call(name, **kwargs)`，HTTP 回调 `POST /api/codeact/tool-call`（`codeact_router.py`，仅凭 run 级短时 `X-CodeAct-Token` 鉴权、不走用户 JWT，路由处声明豁免原因）；`services/codeact_service.py` 负责 token 生命周期（uuid token + 上下文快照写 Redis，TTL=执行超时+60s，执行结束显式吊销）、白名单计算（本次运行挂载的 buildin/knowledge 类工具 − 排除清单 `execute_python`/`ask_user_question`/`install_skill`/`present_artifacts`）与工具分发（按快照重建最小 runtime 注入，权限边界与对话内直接调用一致；单个工具结果 >256KB 报错拒绝不隐式截断）。**中间件**——`execute_python` 仿 `MemoryMiddleware` 用 `StructuredTool.from_function` 在中间件内构建（不进全局工具注册表，无中间件时不可用也不进白名单）；代码与客户端经 `upload_files()`（base64 覆盖写）写入沙盒 `workspace/.codeact/`，执行命令将输出重定向到日志后按退出码选 `head`/`tail`（成功保头部、失败保 traceback 尾部）；连续失败 3 次熔断（成功清零），`awrap_model_call` 向 system prompt 追加使用说明与本次可桥工具清单。新增配置 `codeact_bridge_url`（`CODEACT_BRIDGE_URL`，沙盒内可达的 API 地址）。配套单测 `test_codeact_service.py`（15 例）与 `test_codeact_middleware.py`（13 例）共 28 例全通过；docs/agents 新增 CodeAct 使用说明页。详见 `docs/vibe/2026-07-27-codeact-execution-paradigm.md`。
- 新增 DeepReport 知识库报告流水线 backend（第四种 backend 范式）：`DeepReportAgent` 手工组装固定 StateGraph 五阶段流水线——plan（知识库思维导图 + 结构化输出生成大纲，无知识库/无思维导图 fail-fast）→ review（`interrupt()` 复用 ask_user_question 契约做大纲人工评审，前端零改动；resume 三分支：批准进研究 / 修改意见回 plan 重规划 / 超过 `max_replan` 一次性闸门直接采用当前大纲）→ research_chapter（LangGraph Send API 按章节并行 fan-out，章节内研究/写作双 LLM 分离防幻觉：研究员 `create_agent` 子图挂 `query_kb`/`open_kb_document`/`find_kb_document` + `ToolStrategy(ChapterResearch)` 结构化输出事实清单，写作者纯 LLM 只能引用编号后的 [S#] 事实；单章失败写 confidence=0 占位结果不炸全局）→ synthesize（确定性全局引用重编号 + LLM 补导言/结论，LLM 失败降级确定性模板拼接）→ citation_check（确定性回验 [S#] 标记、剔除无效引用、生成 `citation_report` 与"引用来源"章节，最终报告作为 AIMessage 输出）。新增 `DeepReportContext`（`max_chapters` 默认 8、`max_replan` 默认 2），引用来源复用 `SubAgentSource` 契约，`chapters` 字段经 reducer 按 chapter_id 聚合 Send 分支结果。配套 `test/unit/agents/buildin/test_deepreport.py` 与引用准确率评测脚本 `test/eval/deepreport_citation_eval.py`（`find_file_content` 回验 snippet + ChatbotAgent 基线对比）。**同日完成六项优化迭代**：① 章节进度事件透出——图节点内 `get_stream_writer()` 写 `deepreport_progress` custom 事件（plan/research/synthesize/citation_check 四阶段 started/completed/failed，research 事件携带章节序号与事实数），`agents/base.py` 流式白名单放行 `custom` method，前端复用 chat 界面新增 `DeepReportProgressCard.vue` 进度卡片（`useAgentStreamHandler` 按阶段/章节归并进 `threadState.deepreportProgress`，生成中实时展示四阶段与章节明细）；② 引用语义回验——citation_check 对被引用的 kb_chunk/file 来源（上限 30 条）取 snippet 片段经 `find_file_content` 并发回验，统计写入 `citation_report.source_verification`，未命中来源标注“（未回验）”不删引用，精确匹配 miss 时降级为空白不敏感正则重试（容忍入库时的空白/换行规整差异，减少真实引用误标）；③ confidence 加权——研究员结构化输出新增自评 confidence，章节置信度 = 0.6×自评 + 0.4×min(1, 事实数/6)；④ 合成摘要化——正文改确定性拼接（引用标记不经 LLM），LLM 只看各章摘要结构化产出引言/结论（`ReportFraming`），消除长文场景 LLM 改写 [S#] 风险；⑤ 章节阶段（研究员/写作者）瞬时错误各重试 1 次（研究员 GraphInterrupt 透传；写作失败会作废整章已完成的研究成果，重试收益远大于时延成本）；⑥ “引用来源”章节只列被正文引用的来源，review 静默批准补 warning 日志。单测扩充至 35 用例全通过。详见 `docs/vibe/DeepReport报告流水线-20260727.md`。
- 侧边栏最近聊天记录改为悬浮浮层入口：将"最近聊天记录"从侧边栏 `.fill` 弹性填充区移除，改造成在"创建新对话"导航项行尾添加 History 图标，hover 时弹出 `a-popover` 浮层显示聊天记录。新增 `web/src/components/NewChatNavItem.vue` 组件，封装创建新对话链接 + History 触发器 + Popover 浮层，支持展开态（图标贴在行尾，浮层 `placement="rightBottom"`）和折叠态（History 图标作为独立行显示在 ✚ 下方，浮层 `placement="right"`）两套模板；`ConversationNavSection.vue` 复用，仅移除中键删除避免浮层内误触；`AppLayout.vue` 移除 `.fill` 旧引用与 `.sidebar-conversations`/`.fill` 样式，从 `mainList` 移除"创建新对话"项改为在 `.nav` 顶部独立渲染 `NewChatNavItem`。浮层交互：hover 触发 + `mouseEnterDelay=0.15s` 避免误触 + `mouseLeaveDelay=0.3s` 留出操作窗口，`destroy-on-hide` 保证干净状态；浮层宽度 280px，最大高度 `min(60vh, 480px)` 内部滚动；配色全部用 CSS 变量自动适配明暗主题；History 触发器为 `<button>` + `aria-label`，键盘 Tab 可聚焦触发浮层。功能等价：保留置顶/重命名/删除 ⋮ 菜单、加载更多、置顶指示器、空列表提示，仅移除中键删除。
- 子智能体异步运行改造（Orchestrator-Worker 落地第二阶段）：`task` 工具从父 worker 协程内联执行子智能体图改为独立队列异步执行。**异步执行**——`_create_subagent_run` 创建 `queued` 子 `AgentRun`（`input_payload` 快照 `file_thread_id`/`parent_model`/`tool_call_id` 等执行上下文），经 `enqueue_job("process_subagent_run", run_id, _job_id=f"run:{run_id}", _queue_name=SUBAGENT_QUEUE_NAME)` 入独立子智能体队列（入队失败直接标 failed）；父侧 `_await_subagent_terminal` 轮询等待终态（默认超时 3600s，`context.subagent_timeout_seconds` 可覆盖），超时 best-effort `publish_cancel_signal`，父 run 被取消时级联发布子 run 取消信号；重连/重复 tool call 命中已终结子 run 时直接按终态回传不重入队（幂等复用）。**交付物跨进程传递**——子 worker 写 `agent_runs.output_payload.deliverable`，父侧 `_deliverable_from_output_payload` 还原，校验失败回退空交付物。**可观测性**——新增 `services/subagent_run_worker.py`（`SubAgentWorkerSettings`，max_tries=2），子智能体 `astream` 产出的协议事件以 loading chunk（`thread_id=child_thread_id` + `subagent_tool_call_id`）经 `ChunkedEventWriter` 写入父 run 事件流，状态变更经 custom 事件 `starring.subagent_run_status`（running/completed/cancelled/failed + 可选 error_message）上报。**前端适配**——子线程 loading 轨迹经既有 `resolveChunkThreadId` 路由零改动可用；`useAgentStreamHandler` 新增 `onSubagentRunEvent` 回调，携带 `subagent_tool_call_id` 的 chunk 直接建立 tool_call → child_thread_id 映射（优先于哈希推算，推算保留作兼容），状态事件写入 `subagentRunStatusByToolCall` 驱动状态面板实时反映 completed/failed。配套单测：`test_subagent_task_middleware.py` 按新架构重写（入队契约/终态复用/超时与父取消级联等），新增 `test_subagent_run_worker.py` 8 用例（状态机/事件写入/取消/重试语义），全量 unit 与干净 HEAD 对比无新增失败；docker 端到端验证待后续补做。详见 `docs/vibe/子智能体异步运行改造-20260725.md`。
- 新增 P2 工作流工具生态扩展（tool 节点 / llm 节点挂工具 / application-call 修复）：**tool 节点**——新增第 5 种节点类型 `tool`（`nodes/tool_call.py`，`@register_node("tool")`），不经 LLM 确定性调用单个内置工具（`get_tool_instances_by_category("buildin")`）或 MCP 工具（`get_enabled_mcp_tools(mcp_server)`），按 `tool.name` 匹配（找不到时报错并列出可用工具名）；`config.args` 的 value 支持 `{{ expr }}` 整串表达式从上游节点输出映射（复用 safe_eval，求值上下文与 condition 节点一致，求值失败 fail-fast），结果 dict/list 转 JSON 文本写入 `SubAgentDeliverable`（summary 超 200 字符截断）；`definition.py` 补 tool 节点校验（tool_source ∈ buildin/mcp、tool_name 必填、mcp 来源时 mcp_server 必填、args 为 dict）。**llm 节点挂工具**——config 新增 `tools`（内置工具名列表）/ `mcps`（MCP 服务器 slug 列表）/ `max_tool_steps`（1-25，默认 10）三字段；未配置时保持裸 ainvoke 路径不变（存量定义零回归），配置后经 `resolve_configured_runtime_tools` 解析（解析为空 fail-fast 不退化为裸推理）后走 `create_agent` 受限步数 ReAct 循环（不挂 middleware/checkpointer 保持 fresh context per node，`recursion_limit = max_tool_steps * 2 + 1`）。**application-call 节点修复**——`target_agent_slug` 从“按 backend 类名精确匹配”改为按 `agents.slug` 查库解析（`AgentRepository.get_visible_by_slug`，权限主体为工作流 owner，不存在/无权限 fail-fast，不保留类名直连 fallback），自定义智能体的 `config_json.context` 配置（提示词/工具/MCP/Skill/知识库）经 `normalize_agent_context_config` 规范化后注入目标 context（运行时字段 thread_id/uid/run_id/request_id 最后覆盖）；目标 backend 的 context_schema 含 `workflow_id` 字段（即工作流类）时拒绝嵌套。**存量 import 失效修复**——`_parse_deliverable` 在此前重构中已迁至 `subagent_task.py` 且签名改为 `(messages, artifacts_from_state)`，导致 workflow 节点模块 import 失败、WorkflowBackend 注册被 try/except 吞成 warning；修正 llm/application_call 两节点的导入路径与调用方式。**后端接口补齐**（工作流面向普通用户，而存量工具/MCP 工具列表接口均 admin-only）：新增 `GET /api/workflows/resource-options`（复用 `resolve_agent_resource_options` 返回当前用户可用的 tools/mcps 选项）与 `GET /api/system/mcp-servers/{slug}/tools/enabled`（基于 `get_enabled_mcp_tools`，与运行时一致），均 `get_required_user`。**前端编辑器**：`nodeTypes.js` 新增 tool 节点元数据（Wrench 图标 / success 色系）与默认 config，llm 默认 config 补 tools/mcps；`WorkflowNodeCard.vue` tool 节点摘要行显示工具名、llm 节点挂工具时显示工具数角标；`WorkflowEditorView.vue` 新增 tool 配置面板（来源单选 → 内置工具下拉 / MCP 先选服务器再懒加载工具（reactive 缓存）→ 参数 JSON textarea blur 时 JSON.parse 校验，切换来源清空关联字段）、llm 面板新增挂载工具/MCP 多选与 max_tool_steps 数字输入，application-call 智能体下拉过滤掉 `backend_id === 'WorkflowBackend'` 的智能体（与后端嵌套拒绝对齐）。配套单测：新增 `test_workflow_tool_nodes.py` 23 用例（tool/llm 定义校验、_resolve_args、tool 执行器、llm 工具挂载/向后兼容/解析为空 fail-fast），`test_workflow_nodes.py` 改造 application-call 用例适配 DB 解析并补嵌套拒绝/配置注入用例，两文件 37 用例全部通过；eslint + prettier 通过。
