...
StarRing 的工具系统基于注册机制，支持多种工具类型的动态组装。

## 工具注册机制

StarRing 的工具系统采用 `@tool` 装饰器注册机制，核心位于 `backend/package/StarRing/agents/toolkits/registry.py`。

### @tool 装饰器

```python
from StarRing.agents.toolkits.registry import tool
...
```python
from StarRing.agents.toolkits import buildin, mysql  # 触发 @tool 装饰器执行
```
...
```python
from StarRing.agents.toolkits.kbs import get_common_kb_tools
