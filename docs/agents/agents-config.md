...
StarRing 的智能体系统基于 LangGraph 构建。对开发者来说，最重要的不是单独理解某个页面或某个字段，而是理解三件事：
...
建议在 `backend/package/StarRing/agents` 下按包组织一个智能体：

```text
backend/package/StarRing/agents/
└── my_agent/
    ├── __init__.py
    ├── context.py
    └── graph.py
...
```python
from StarRing.agents import BaseAgent, BaseContext, load_chat_model
...
`BaseContext` 定义在 `backend/package/StarRing/agents/context.py`，它不是一个普通的数据类，而是整个智能体配置链路的核心：
...
from StarRing.agents import BaseContext
...
因此 Graph 不是和 Context 解耦的。相反，Graph 的构造本身就依赖 Context。普通 Agent 在归一化后的 `context.subagents` 非空时会挂载 StarRing 的 task middleware；`SubAgentBackend` 自身隐藏并清空 `subagents` 字段，因此子智能体不会继续调用子智能体。
...
像 todo 这类运行态信息，不建议再放进 `capabilities`。StarRing 当前会直接从 LangGraph state 中提取 `agent_state`，前端在创建对话后常态化展示状态入口，并在状态面板中渲染 `todos`、`files`、`artifacts`、`subagent_runs` 等运行时内容。
