...
StarRing 支持完整的品牌自定义，包括 Logo、组织名称、版权信息、登录协议等，方便企业用户进行品牌定制。
...
```bash
cp backend/package/StarRing/config/static/info.template.yaml backend/package/StarRing/config/static/info.local.yaml
```

### 步骤 2：编辑品牌信息

在 `backend/package/StarRing/config/static/info.local.yaml` 中配置你的品牌信息：
...
```env
StarRing_BRAND_FILE_PATH=backend/package/StarRing/config/static/info.local.yaml
```
...
在 `backend/package/StarRing/config/static/info.local.yaml` 的 `footer` 下新增以下字段：
