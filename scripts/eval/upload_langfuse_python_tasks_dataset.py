...
PYTHON_TASK_ITEMS = [
    ...
    },
    {
        "id": "py-task-006",
        "input": (
            "请用 Python 完成任务并给出最终答案：给定 s = 'starring Agent Evaluation'，"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--dataset-name",
        default=f"starring-python-tasks-{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}",
...
def main() -> None:
    ...
    client.create_dataset(
        ...
        description="starring agent evaluation dataset: deterministic Python programming tasks.",
        metadata={
    ...
    for item in PYTHON_TASK_ITEMS:
        client.create_dataset_item(
            ...
            metadata={"category": "python_programming", "source": "starring_eval_smoke"},
        )
    client.flush()
