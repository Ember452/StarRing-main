...
# starring Initialization Script for Bash/Linux/macOS
...
ensure_jwt_env() {
    if grep -Eq '^JWT_SECRET_KEY=.+' .env && grep -Eq '^STARRING_INSTANCE_ID=.+' .env; then
        return
    fi
    ...
    read -p "Please enter your STARRING_INSTANCE_ID (press Enter to auto-generate): " STARRING_INSTANCE_ID
    if [ -z "$STARRING_INSTANCE_ID" ]; then
        STARRING_INSTANCE_ID="instance-$(generate_hex 8)"
        echo "Generated STARRING_INSTANCE_ID and saved it to .env."
    fi
...
echo "🚀 Initializing starring project..."
echo "=================================="
