...
function Ensure-JwtEnv {
    if ((Test-EnvValue "JWT_SECRET_KEY") -and (Test-EnvValue "STARRING_INSTANCE_ID")) {
        return
    }
    ...
    $STARRING_INSTANCE_ID = Read-Host "Please enter your STARRING_INSTANCE_ID (press Enter to auto-generate)"
    if ([string]::IsNullOrEmpty($STARRING_INSTANCE_ID)) {
        $STARRING_INSTANCE_ID = "instance-$(New-RandomHex 8)"
        Write-Host "Generated STARRING_INSTANCE_ID and saved it to .env." -ForegroundColor Green
    }
...
STARRING_INSTANCE_ID=$STARRING_INSTANCE_ID
"@ | Add-Content -Path ".env" -Encoding UTF8
}

Write-Host "🚀 Initializing starring project..." -ForegroundColor Cyan
...
"@
    ...
    Remove-Variable -Name "STARRING_INSTANCE_ID" -ErrorAction SilentlyContinue
}
