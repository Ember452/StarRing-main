...
from starring_cli.client import ClientError, StarRingClient
from starring_cli.config import ConfigStore, Remote
from starring_cli.discovery import ServerCompatibilityError, ensure_server_compatible
...
def remote_ping(store: ConfigStore, name: str | None, console: Console, client_factory=StarRingClient) -> None:
    config = store.load()
...
def whoami(store: ConfigStore, remote_name: str | None, console: Console, client_factory=StarRingClient) -> None:
    config = store.load()
    remote = config.get_remote(remote_name)
...
def status(store: ConfigStore, remote_name: str | None, console: Console, client_factory=StarRingClient) -> None:
    config = store.load()
    remote = config.get_remote(remote_name)
    with client_factory(remote) as client:
...
def _ensure_server_compatible(client: StarRingClient, required_capability: str) -> None:
    try:
        discovery = client.discovery()
    except ClientError as exc:
        raise CommandError(f"无法读取服务端 discovery，请确认远程是 starring 0.7.1 或更高版本: {exc}") from exc
    try:
