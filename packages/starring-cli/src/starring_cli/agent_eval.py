...
from starring_cli.client import StarRingClient
from starring_cli.config import ConfigStore
...
def run_langfuse_agent_experiment(
    ...
    *,
    ...
    if not remote.api_key:
        raise AgentEvalError(f"remote 尚未登录: {remote.name}。请先运行 starring login。")
    ...
    experiment_name = options.experiment_name or f"starring-agent-eval-{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}"
