...
def add_uploaded_documents(client: StarRingClient, kb_id: str, uploaded: list[UploadResult]) -> dict:
...
def _ensure_kb_upload_supported(client: StarRingClient) -> None:
    try:
...
def _load_kb_types(client: StarRingClient) -> dict:
...
def _resolve_database(client: StarRingClient, kb_id: str | None, kb_types: dict, console: Console) -> dict:
...
def _list_uploadable_databases(client: StarRingClient, kb_types: dict) -> list[dict]:
...
def _load_supported_extensions(client: StarRingClient) -> set[str]:
