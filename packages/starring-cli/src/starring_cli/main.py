...
from STARRING_cli import __version__
from starring_cli.agent_eval import AgentEvalError, AgentEvalOptions, run_langfuse_agent_experiment
from starring_cli.client import ClientError
from starring_cli.commands import (
    CommandError,
    login_with_api_key,
    login_with_browser,
...
from starring_cli.config import ConfigError, ConfigStore
from starring_cli.kb_upload import KbUploadError, KbUploadOptions, run_kb_upload

console = Console()
app = typer.Typer(help="starring command line client.", invoke_without_command=True)
remote_app = typer.Typer(help="Manage starring remotes.")
agent_app = typer.Typer(help="Run and manage starring agents.")
...
def eval_agent(
    ...
    agent_slug: str = typer.Option(..., "--agent-slug", help="starring agent slug."),
    ...
    timeout_seconds: float = typer.Option(900, "--timeout-seconds", help="Per item starring API timeout."),
):
