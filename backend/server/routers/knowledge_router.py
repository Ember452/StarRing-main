...
from starring import config, knowledge_base
from starring.knowledge.factory import KnowledgeBaseFactory
from starring.knowledge.graphs.milvus_graph_service import GRAPH_TASK_TYPE, MilvusGraphService
from starring.knowledge.parser import SUPPORTED_FILE_EXTENSIONS, Parser, is_supported_file_extension
from starring.knowledge.utils import calculate_content_hash, is_minio_url, parse_minio_url
from starring.knowledge.utils.mindmap_utils import (
...
from starring.knowledge.utils.sample_question_utils import (
...
from starring.services.task_service import TaskContext, tasker
from starring.services.workspace_service import MAX_WORKSPACE_UPLOAD_SIZE_BYTES, resolve_workspace_file_path
from starring.storage.minio.client import MinIOClient, StorageError, aupload_file_to_minio, get_minio_client
from starring.storage.postgres.models_business import User
from starring.utils import logger
from starring.utils.upload_utils import MAX_UPLOAD_SIZE_BYTES, read_upload_with_limit, write_upload_to_path
...
async def _require_kb_manager(kb_id: str, current_user: User) -> None:
    """管理类操作权限：管理员，或知识库 owner（created_by 本人）。"""
    ...
    if kb.created_by != current_user.uid:
        raise HTTPException(status_code=403, detail="没有权限管理该知识库")


async def _require_kb_access(kb_id: str, current_user: User) -> None:
    """查看类操作权限：管理员，或按 share_config 可访问该知识库的用户。"""
    if current_user.role in ("admin", "superadmin"):
        return
    user_info = {
...
async def get_database_mindmap_files(kb_id: str, current_user: User = Depends(get_required_user)):
    """获取指定知识库的所有文件列表。"""
    await _require_kb_access(kb_id, current_user)
    try:
...
async def generate_mindmap(
    ...
    await _require_kb_manager(kb_id, current_user)
    try:
...
async def get_database_mindmap(kb_id: str, current_user: User = Depends(get_required_user)):
    """获取知识库关联的思维导图。"""
    await _require_kb_access(kb_id, current_user)
    try:
...
async def get_mindmap_diff_route(kb_id: str, current_user: User = Depends(get_required_user)):
    """检测思维导图与知识库文件的变更差异。"""
    await _require_kb_access(kb_id, current_user)
    try:
...
async def update_database_info(
    ...
    """更新知识库信息（管理员或知识库 owner）

    共享设置（share_config）涉及可见范围提升，仍仅限管理员修改。
    """
    await _require_kb_manager(kb_id, current_user)
    if data.share_config is not None and current_user.role not in ("admin", "superadmin"):
        raise HTTPException(status_code=403, detail="共享设置仅管理员可修改")
    logger.debug(
...
async def delete_database(kb_id: str, current_user: User = Depends(get_required_user)):
    """删除知识库（管理员或知识库 owner）"""
    await _require_kb_manager(kb_id, current_user)
...
async def get_graph_build_status(kb_id: str, current_user: User = Depends(get_required_user)):
    await _require_kb_access(kb_id, current_user)
    try:
...
async def index_graph_build(
    ...
    try:
        ...
        # 异步构建知识图谱，后台构建，用户不用立即看到，但可以立即收到反馈
