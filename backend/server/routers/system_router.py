...
from starring import config, get_version
...
async def get_system_logs(levels: str | None = None, current_user: User = Depends(get_admin_user)):
    ...
    """
    try:
        from starring.utils.logging_config import LOG_FILE
...
async def load_info_config():
    ...
    try:
        # 配置文件路径
        brand_file_path = os.environ.get("STARRING_BRAND_FILE_PATH", "package/starring/config/static/info.local.yaml")
        config_path = Path(brand_file_path)

        # 检查文件是否存在
        if not config_path.exists():
            logger.debug(f"The config file {config_path} does not exist, using default config")
            config_path = Path("package/starring/config/static/info.template.yaml")
        ...
        # 注入版本号占位符
        content = content.replace("{{STARRING_VERSION}}", get_version())
