...
from starring.utils.datetime_utils import UTC, ensure_shanghai, shanghai_now, utc_now
...
async def get_all_conversations(
    ...
    """获取所有对话（管理员权限）"""
    from starring.storage.postgres.models_business import Conversation, ConversationStats
...
async def get_user_activity_stats(
    ...
    """获取用户活动统计（管理员权限）"""
    try:
        from starring.storage.postgres.models_business import Conversation, User
...
async def get_tool_call_stats(
    ...
    """获取工具调用统计（管理员权限）"""
    try:
        from starring.storage.postgres.models_business import ToolCall
...
async def get_agent_analytics(
    ...
    """获取智能体分析（管理员权限）"""
    try:
        from starring.storage.postgres.models_business import Conversation, Message, MessageFeedback, ToolCall
...
async def get_all_feedbacks(
    ...
    """获取所有反馈记录（管理员权限）"""
    from starring.storage.postgres.models_business import Conversation, Message, MessageFeedback, User
...
async def get_call_timeseries_stats(
    ...
    """获取调用分析时间序列统计（管理员权限）"""
    try:
        from starring.storage.postgres.models_business import Conversation, Message, ToolCall
