...
class TestMinIOClientStatFile:
    ...
    def test_stat_file_returns_size(self):
        from starring.storage.minio.client import MinIOClient
...
class TestAddFileRecordSizeFallback:
    ...
    def _make_test_kb(self, work_dir="/tmp/test_kb"):
        from starring.knowledge.base import KnowledgeBase
    ...
    async def test_add_file_record_fetches_size_from_minio_when_missing(self):
        ...
        with patch("starring.storage.minio.get_minio_client", return_value=mock_minio):
...
async def test_save_metadata_does_not_overwrite_existing_kb_config():
    ...
    with (
        patch("starring.repositories.knowledge_base_repository.KnowledgeBaseRepository", return_value=kb_repo),
        patch("starring.repositories.knowledge_file_repository.KnowledgeFileRepository", return_value=SimpleNamespace()),
        patch("starring.repositories.evaluation_repository.EvaluationRepository", return_value=SimpleNamespace()),
    ):
