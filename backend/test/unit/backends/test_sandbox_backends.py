...
from starring.agents.backends.composite import (
    CustomCompositeBackend,
    create_agent_composite_backend,
    create_agent_filesystem_middleware,
)
from starring.agents.backends.sandbox import resolve_virtual_path, sandbox_id_for_thread
...
def test_create_agent_composite_backend_uses_prepared_readable_skills(monkeypatch):
    monkeypatch.setattr("starring.agents.backends.sandbox.backend.get_sandbox_provider", lambda: object())
...
def test_provider_uses_distinct_sandbox_scope_for_different_uid(monkeypatch) -> None:
    from starring.agents.backends.sandbox.provider import ProvisionerSandboxProvider
    ...
    monkeypatch.setattr("starring.agents.backends.sandbox.provider.load_user_agent_env", lambda uid: {"A": uid})
...
def test_provisioner_uses_file_and_skills_thread_ids(monkeypatch) -> None:
    ...
    monkeypatch.setattr("starring.agents.backends.sandbox.backend.get_sandbox_provider", lambda: FakeProvider())
    monkeypatch.setattr(
        "starring.agents.backends.sandbox.backend.sync_thread_readable_skills",
