...
from starring.models.chat import LangChainChatAdapter, select_model
from starring.models.embed import OtherEmbedding, select_embedding_model
from starring.models.rerank import OpenAIReranker, get_reranker
from starring.models.providers.cache import ModelInfo
...
def test_resolve_chat_model_spec_prefers_explicit_then_fallback_then_default(monkeypatch):
    monkeypatch.setattr("starring.agents.models.sys_config.default_model", "system-default:model")
...
def test_resolve_chat_model_spec_rejects_all_empty(monkeypatch):
    monkeypatch.setattr("starring.agents.models.sys_config.default_model", "")
...
def test_select_embedding_model_loads_model_from_cache(monkeypatch):
    monkeypatch.setattr(
        "starring.models.embed.model_cache.get_model_info",
...
def test_select_model_wraps_langchain_model_and_expands_model_params(monkeypatch):
    ...
    monkeypatch.setattr(
        "starring.models.chat.model_cache.get_model_info",
    ...
    monkeypatch.setattr("starring.models.chat.load_chat_model", fake_load_chat_model)
...
def test_select_model_maps_anthropic_max_completion_tokens(monkeypatch):
    ...
    monkeypatch.setattr(
        "starring.models.chat.load_chat_model",
...
def test_load_chat_model_uses_toolcall_chunk_fix_for_openai_compatible(monkeypatch):
    from starring.agents.models import _ToolCallChunkFixChatOpenAI

    monkeypatch.setattr(
        "starring.agents.models.model_cache.get_model_info",
...
def test_embedding_sync_400_logs_warning(monkeypatch):
    ...
    monkeypatch.setattr("starring.models.embed.requests.post", fake_post)
...
def test_embedding_sync_429_retries_ten_times_before_success(monkeypatch):
    warnings = _capture_embed_warnings(monkeypatch)
    sleeps = []
    monkeypatch.setattr("starring.models.embed.time.sleep", sleeps.append)
    ...
    monkeypatch.setattr("starring.models.embed.requests.post", lambda *_args, **_kwargs: responses.pop(0))
...
async def test_embedding_async_400_logs_warning(monkeypatch):
    ...
    monkeypatch.setattr("starring.models.embed.httpx.AsyncClient", FakeAsyncClient)
...
async def test_embedding_async_429_retries_ten_times_before_success(monkeypatch):
    ...
    monkeypatch.setattr("starring.models.embed.asyncio.sleep", fake_sleep)
...
def test_get_reranker_loads_model_from_cache(monkeypatch):
    monkeypatch.setattr(
        "starring.models.rerank.model_cache.get_model_info",
