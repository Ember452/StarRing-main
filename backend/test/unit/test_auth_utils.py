...
from starring.utils.auth_utils import JWT_ALGORITHM, JWT_AUDIENCE, AuthUtils
...
def test_access_token_contains_instance_claims(monkeypatch):
    ...
    monkeypatch.setenv("STARRING_INSTANCE_ID", "pytest-instance")

    token = AuthUtils.create_access_token({"sub": "1"})
    payload = AuthUtils.verify_access_token(token)

    assert payload["sub"] == "1"
    assert payload["iss"] == "starring-know:pytest-instance"
    assert payload["aud"] == JWT_AUDIENCE


def test_access_token_auto_generates_dev_secret(monkeypatch):
    monkeypatch.setenv("STARRING_ENV", "development")
...
def test_access_token_requires_configured_secret_in_production(monkeypatch):
    monkeypatch.setenv("STARRING_ENV", "production")
    monkeypatch.delenv("JWT_SECRET_KEY", raising=False)
    monkeypatch.setenv("STARRING_INSTANCE_ID", "pytest-instance")
...
def test_access_token_rejects_public_default_secret_in_production(monkeypatch):
    monkeypatch.setenv("STARRING_ENV", "production")
    monkeypatch.setenv("JWT_SECRET_KEY", "STARRING_know_secure_key")
    monkeypatch.setenv("STARRING_INSTANCE_ID", "pytest-instance")
...
def test_access_token_auto_generates_dev_instance_id(monkeypatch):
    monkeypatch.setenv("STARRING_ENV", "development")
    monkeypatch.setenv("JWT_SECRET_KEY", "test-secret-key-with-enough-randomness")
    monkeypatch.delenv("STARRING_INSTANCE_ID", raising=False)

    token = AuthUtils.create_access_token({"sub": "1"})

    assert AuthUtils.verify_access_token(token)["iss"].startswith("starring-know:instance-")
    assert os.environ["STARRING_INSTANCE_ID"].startswith("instance-")
...
def test_access_token_requires_instance_id_in_production(monkeypatch):
    ...
    with pytest.raises(ValueError, match="STARRING_INSTANCE_ID"):
        AuthUtils.create_access_token({"sub": "1"})
...
def test_verify_access_token_rejects_wrong_issuer(monkeypatch):
    ...
    monkeypatch.setenv("STARRING_INSTANCE_ID", "pytest-instance")
    token = jwt.encode(
        {"sub": "1", "exp": utc_now() + timedelta(minutes=5), "iss": "starring-know:other", "aud": JWT_AUDIENCE},
...
def test_verify_access_token_rejects_wrong_audience(monkeypatch):
    ...
    monkeypatch.setenv("STARRING_INSTANCE_ID", "pytest-instance")
    token = jwt.encode(
        {"sub": "1", "exp": utc_now() + timedelta(minutes=5), "iss": "starring-know:pytest-instance", "aud": "other-api"},
        "test-secret-key-with-enough-randomness",
        algorithm=JWT_ALGORITHM,
    )
