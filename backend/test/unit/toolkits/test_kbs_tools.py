...
from starring.agents.toolkits.kbs import tools
...
async def test_find_kb_document_returns_context_windows(monkeypatch) -> None:
    ...
    monkeypatch.setattr(tools._get_knowledge_base(), "find_file_content", _fake_find_file_content)
...
async def test_open_kb_document_reads_markdown_content_by_default_window(monkeypatch) -> None:
    ...
    monkeypatch.setattr(tools._get_knowledge_base(), "open_file_content", _fake_open_file_content)
