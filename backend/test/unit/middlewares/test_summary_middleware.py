...
from starring.agents.middlewares.summary import (
    StarRingSummarizationMiddleware,
...
@pytest.mark.unit
def test_create_summary_middleware_uses_deepagents_with_STARRING_outputs_root() -> None:
    ...
    assert isinstance(middleware, StarRingSummarizationMiddleware)
...
def test_wrap_model_call_keeps_summary_keep_messages_inline_when_summary_triggers() -> None:
    ...
    middleware = StarRingSummarizationMiddleware(
        model=model,
        backend=backend,
        trigger=("tokens", 500),
        keep=("messages", 3),
