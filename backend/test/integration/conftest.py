...
SANDBOX_CONTAINER_PREFIX = os.getenv("STARRING_SANDBOX_CONTAINER_PREFIX", "starring-sandbox")
...
def _docker_api_request(method: str, path: str) -> list[dict] | dict:
    cmd = [
        ...
        "-sS",
        "--unix-socket",
        os.getenv("STARRING_DOCKER_API_SOCKET", "/var/run/docker.sock"),
        "-X",
        method,
        f"{os.getenv('STARRING_DOCKER_API_BASE', 'http://localhost').rstrip('/')}{path}",
    ]
