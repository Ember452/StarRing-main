...
from starring.agents.mcp.service import get_enabled_mcp_server_slugs
from starring.agents.skills.repository import SkillRepository
from starring.storage.postgres.models_business import Skill, User
from starring.utils.logging_config import logger
from starring.utils.share_config import SHARE_ACCESS_LEVELS, normalize_share_config
...
def get_builtin_skill_specs() -> list[Any]:
    from starring.agents.skills.buildin import BUILTIN_SKILLS
...
async def get_skill_dependency_options(
    ...
    from starring.agents.toolkits.service import get_tool_metadata
...
async def _stage_skill_draft_item(
    ...
    *,
    ...
    await asyncio.to_thread(shutil.copytree, source_skill_dir, item_dir, symlinks=False)
...
async def _import_skill_dir_impl(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix=".skill-import-", dir=str(skills_root.parent)) as temp_root:
        ...
        await asyncio.to_thread(shutil.copytree, source_skill_dir, stage_dir)
        ...
        await asyncio.to_thread(shutil.move, str(stage_dir), str(temp_target))
        ...
        await asyncio.to_thread(temp_target.rename, final_dir)
