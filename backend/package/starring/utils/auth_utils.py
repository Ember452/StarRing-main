...
PUBLIC_DEFAULT_JWT_SECRET_KEY = "STARRING_know_secure_key"
PASSWORD_HASHER = PasswordHasher()


def _is_production_env() -> bool:
    return os.environ.get("STARRING_ENV", "development").strip().lower() in {"prod", "production"}
...
def _get_jwt_issuer() -> str:
    instance_id = _get_or_create_dev_env("STARRING_INSTANCE_ID", lambda: f"instance-{secrets.token_hex(8)}")
    return f"starring-know:{instance_id}"
