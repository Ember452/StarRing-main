...
// 流式渲染节流：content 频繁变化（如 Agent 流式输出）时合并渲染，
// 避免每帧都重新解析 markdown（含 shiki 高亮 + DOMPurify 消毒）造成卡顿。
// - RENDER_DEBOUNCE_MS：变化停止后多久触发渲染
// - RENDER_MAX_INTERVAL_MS：持续变化时最多间隔多久强制渲染一次，保证用户可见更新
const RENDER_DEBOUNCE_MS = 80
const RENDER_MAX_INTERVAL_MS = 300
let pendingContent = ''
let pendingTheme = ''
...
const runRender = async (content, theme, version) => {
  if (!content) {
    if (version === renderVersion) renderedMarkdown.value = ''
    return
  }
  const html = await renderMarkdown(content, { theme })
  // 仅当这次渲染仍是最新的版本时才更新，避免旧渲染结果覆盖新结果
  if (version === renderVersion) {
    renderedMarkdown.value = html
  }
}

const scheduleRender = (content, theme) => {
  ...
  if (elapsed >= RENDER_MAX_INTERVAL_MS) {
    lastRenderAt = Date.now()
    renderVersion += 1
    runRender(content, theme, renderVersion)
    return
  }
  ...
  debounceTimer = setTimeout(() => {
    ...
    lastRenderAt = Date.now()
    renderVersion += 1
    runRender(pendingContent, pendingTheme, renderVersion)
  }, RENDER_DEBOUNCE_MS)
}
