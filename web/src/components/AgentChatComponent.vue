...
// 缓存历史对话的 displayItems：conv 引用不变时复用，避免流式更新时
// 重新计算历史消息的展示项、触发历史消息组件不必要重渲染。
const historyDisplayItemsCache = new WeakMap()

const getDisplayItems = (conv) => {
  // 流式对话每次重新计算
  if (conv.status === 'streaming') {
    return getConversationDisplayItems(conv, { enrichToolCalls: getMessageToolCalls })
  }
  // 历史对话使用缓存
  let cached = historyDisplayItemsCache.get(conv)
  if (!cached) {
    cached = getConversationDisplayItems(conv, { enrichToolCalls: getMessageToolCalls })
    historyDisplayItemsCache.set(conv, cached)
  }
  return cached
}
