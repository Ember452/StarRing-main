...
class LocalContainerProvisionerBackend:
    def __init__(self):
        ...
        self._container_prefix = os.getenv("DOCKER_SANDBOX_PREFIX", "starring-sandbox")
    ...
    def list(self) -> list[SandboxRecord]:
        containers = self._client.containers.list(
            all=True, filters={"label": ["app=starring-sandbox", "managed-by=starring-sandbox-provisioner"]}
        )
...
class KubernetesProvisionerBackend:
    def __init__(self):
        ...
        self._lock = threading.Lock()
        self._namespace = os.getenv("K8S_NAMESPACE", "starring-know")
        self._sandbox_image = os.getenv(
            "SANDBOX_IMAGE",
            "enterprise-public-cn-beijing.cr.volces.com/vefaas-public/all-in-one-sandbox:latest",
        )
        self._skill_pvc = os.getenv("SKILLS_PVC", "starring-skills")
        self._thread_pvc = os.getenv("THREAD_PVC", "starring-thread")
        ...
        *,
        ...
        return self._client.V1Pod(
            metadata=self._client.V1ObjectMeta(
                name=pod_name,
                labels={"app": "starring-sandbox", "sandbox-id": sandbox_id},
                annotations={
                    "thread-id": thread_id,
    ...
    def list(self) -> list[SandboxRecord]:
        ...
        try:
            pod_list = self._core_api.list_namespaced_pod(
                namespace=self._namespace,
                label_selector="app=starring-sandbox",
            )
        except ApiException:
            return []
...
app = FastAPI(title="starring Sandbox Provisioner", lifespan=lifespan)
